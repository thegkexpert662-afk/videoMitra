...
class _EditorScreenState extends State<EditorScreen> {
  ...
  void initState() {
    ...
    if (widget.project != null) { editor.project = widget.project!; } else { editor.initialize(widget.mediaFiles); }
  ...
  Future<void> _openClip(int index, {bool autoSeek = false}) async {
    ...
    if (clip.kind == MediaKind.image) { editor.selectedClip = index; if (mounted) setState(() => loading = false); return; }
    try {
      ...
      } else { await p.seekTo(clip.start); }
      if (mounted) setState(() => loading = false);
    } catch (e) { if (mounted) setState(() { loading = false; error = 'Video load failed: $e'; }); }
  }

  void _playerTick() {
    final p = player; if (p == null || !p.value.isInitialized || !mounted || editor.project.clips.isEmpty) return;
    ...
    if (source >= clip.end && p.value.isPlaying) {
      if (editor.selectedClip + 1 < editor.project.clips.length) { _openClip(editor.selectedClip + 1); Future.microtask(() => player?.play()); } else { p.pause(); }
    }
  }
  ...
  String time(Duration d) { final h=d.inHours; final m=d.inMinutes.remainder(60).toString().padLeft(2,'0'); final s=d.inSeconds.remainder(60).toString().padLeft(2,'0'); return h>0?'$h:$m:$s':'$m:$s'; }
  double get ratio { switch(editor.project.ratio){case CanvasRatio.r9x16:return 9/16;case CanvasRatio.r16x9:return 16/9;case CanvasRatio.r1x1:return 1;case CanvasRatio.r4x3:return 4/3;case CanvasRatio.r4x5:return 4/5;case CanvasRatio.original:return player?.value.isInitialized==true?player!.value.aspectRatio:16/9;} }
  Color? color(String? v){if(v==null)return null;final h=v.replaceAll('#','');if(h.length!=6)return null;return Color(int.parse('FF$h',radix:16));}

  Widget _preview() {
    final clip=editor.project.clips.isEmpty?null:editor.project.clips[editor.selectedClip];
    Widget media;
    if(clip==null) media=const Center(child:Text('Import media to start'));
    else if(clip.kind==MediaKind.image) media=Image.file(clip.file,fit:BoxFit.contain,errorBuilder:(_,__,___)=>const Center(child:Text('Image load failed')));
    else if(loading) media=const Center(child:CircularProgressIndicator());
    else if(error!=null) media=Center(child:Padding(padding:const EdgeInsets.all(20),child:Text(error!,textAlign:TextAlign.center)));
    else if(player?.value.isInitialized==true) media=FittedBox(fit:BoxFit.contain,child:SizedBox(width:player!.value.size.width,height:player!.value.size.height,child:VideoPlayer(player!)));
    else media=const Center(child:CircularProgressIndicator());
    media=_filterPreview(media);
    ...
    final vfx=active.where((e)=>e.kind==ElementKind.vfx).toList();
    for(final e in vfx){ if(e.text.toLowerCase()=='blur') media=BackdropFilter(filter:ui.ImageFilter.blur(sigmaX:8*e.intensity,sigmaY:8*e.intensity),child:media); if(e.text.toLowerCase()=='flash') media=Stack(children:[media,Container(color:Colors.white.withOpacity(.35*e.intensity))]); if(e.text.toLowerCase()=='zoom') media=Transform.scale(scale:1+.12*e.intensity,child:media); }
    return Center(child:AspectRatio(aspectRatio:ratio,child:ClipRect(child:Stack(fit:StackFit.expand,children:[Container(color:color(editor.project.backgroundColor)??Colors.black),media,for(final e in active.where((e)=>e.kind!=ElementKind.vfx))_element(e),if(exporting)Container(color:Colors.black54,child:const Center(child:CircularProgressIndicator()))]))));
  }

  Widget _filterPreview(Widget child){
    if(editor.project.filter==FilterPreset.blackWhite)return ColorFiltered(colorFilter:const ColorFilter.matrix([.2126,.7152,.0722,0,0,.2126,.7152,.0722,0,0,.2126,.7152,.0722,0,0,0,0,0,1,0]),child:child);
    final a=editor.project.adjustments; final b=(a['brightness']??0)/100; final c=1+(a['contrast']??0)/100; final s=1+(a['saturation']??0)/100;
    if(editor.project.filter==FilterPreset.bright||b!=0||c!=1||s!=1)return ColorFiltered(colorFilter:ColorFilter.matrix([c,0,0,0,b*255,0,c,0,0,b*255,0,0,c,0,b*255,0,0,0,1,0]),child:child);
    return child;
  }

  Widget _element(EditorElement e){
    Widget child;
    if(e.kind==ElementKind.text)child=Text(e.text,textAlign:TextAlign.center,style:TextStyle(color:color(e.color)??Colors.white,fontSize:e.fontSize*e.scale,fontWeight:e.bold?FontWeight.bold:FontWeight.normal,fontStyle:e.italic?FontStyle.italic:FontStyle.normal,shadows:e.shadow?const[Shadow(blurRadius:5,offset:Offset(2,2))]:null));
    else if(e.kind==ElementKind.image&&e.assetPath!=null)child=Image.file(File(e.assetPath!),fit:BoxFit.contain);
    else child=Text(e.text.isEmpty?'✨':e.text,style:TextStyle(fontSize:48*e.scale));
  ...
  Future<void> _speed() async {const v=[.25,.5,.75,1,1.25,1.5,2];final s=await showModalBottomSheet<double>(context:context,backgroundColor:const Color(0xFF111118),builder:(_)=>Wrap(children:[for(final x in v)SizedBox(width:MediaQuery.sizeOf(context).width/3,child:ListTile(title:Text('${x}x',textAlign:TextAlign.center),onTap:()=>Navigator.pop(context,x)))]));if(s!=null){editor.setSpeed(s);await player?.setPlaybackSpeed(s);}}
  ...
  Future<void> _keyframe() async {editor.addKeyframe();_snack('Keyframe added');}
  Future<void> _chroma() async {final v=await showDialog<double>(context:context,builder:(_)=>AlertDialog(backgroundColor:const Color(0xFF17171F),title:const Text('Green Screen'),content:const Text('Green color key is used for export. Adjust similarity strength.'),actions:[TextButton(onPressed:()=>Navigator.pop(context),child:const Text('Cancel')),FilledButton(onPressed:()=>Navigator.pop(context,.55),child:const Text('Use Green Key'))]));if(v!=null)editor.addElement(EditorElement(id:'chroma_${DateTime.now().millisecondsSinceEpoch}',kind:ElementKind.vfx,text:'Chroma',start:Duration.zero,end:editor.project.duration,intensity:v));}
