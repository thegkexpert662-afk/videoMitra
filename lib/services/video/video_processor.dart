...
class VideoProcessor {
  Future<VideoProcessResult> trim(EditorClip clip) async {
    ...
    return ReturnCode.isSuccess(code) && await out.exists()
        ? VideoProcessResult(true, file: out)
        : const VideoProcessResult(false, error: 'Video processing failed');
  }
  ...
  Future<VideoProcessResult> render({required ProjectModel project, required int width, required int height, required int fps}) async {
    ...
    final videoInput = labels.join('');
    final videoOut = '[vmv]';
    filters.add('$videoInput' + 'concat=n=${project.clips.length}:v=1:a=0' + videoOut);

    args.addAll(['-filter_complex', filters.join(';'), '-map', videoOut]);

    if (project.clips.length == 1 && project.audioTracks.isEmpty && !project.clips.first.muted && project.filter == FilterPreset.normal && project.adjustments.isEmpty) {
      args.addAll(['-map', '0:a?']);
    } else if (project.audioTracks.isNotEmpty) {
      for (final audio in project.audioTracks) {
        args.addAll(['-i', _q(audio.file.path)]);
      }
    ...
    return ReturnCode.isSuccess(code) && await out.exists()
        ? VideoProcessResult(true, file: out)
        : const VideoProcessResult(false, error: 'Export failed. Try a lower resolution or shorter project.');
  }
  ...
  String _videoFilter(ProjectModel p) {
    ...
    switch (p.filter) {
      case FilterPreset.normal:
        break;
      case FilterPreset.bright:
        parts.add('eq=brightness=${0.18 * intensity}:contrast=1.05');
        break;
      case FilterPreset.contrast:
        parts.add('eq=contrast=${1 + .55 * intensity}');
        break;
      case FilterPreset.warm:
        parts.add('colorbalance=rs=${.18 * intensity}:gs=${.05 * intensity}:bs=${-.12 * intensity}');
        break;
      case FilterPreset.cool:
        parts.add('colorbalance=rs=${-.10 * intensity}:gs=${.04 * intensity}:bs=${.20 * intensity}');
        break;
      case FilterPreset.vintage:
        parts.add('curves=vintage');
        break;
      case FilterPreset.blackWhite:
        parts.add('hue=s=0');
        break;
      case FilterPreset.cinematic:
        parts.add('eq=contrast=${1 + .25 * intensity}:saturation=${1 + .15 * intensity},vignette=PI/5');
        break;
      case FilterPreset.dramatic:
        parts.add('eq=contrast=${1 + .7 * intensity}:saturation=${1 + .1 * intensity},unsharp=5:5:${1.2 * intensity}:5:5:0');
        break;
    }
    final a = p.adjustments;
    if (a.isNotEmpty) {
      parts.add('eq=brightness=${(a['brightness'] ?? 0) / 100}:contrast=${1 + (a['contrast'] ?? 0) / 100}:saturation=${1 + (a['saturation'] ?? 0) / 100}');
    }
